...
<form action="{{ route('laravel-permission-gui::permissions.store') }}" method="post" role="form">
    ...
    <a class="btn btn-labeled btn-default" href="{{ route('laravel-permission-gui::permissions.index') }}"><span class="btn-label"><i class="fa fa-chevron-left"></i></span>{{ trans('laravel-permission-gui::button.cancel') }}</a>
</form>
@endsection
